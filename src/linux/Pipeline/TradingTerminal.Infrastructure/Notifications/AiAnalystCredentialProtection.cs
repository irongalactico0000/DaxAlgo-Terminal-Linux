...
/// <summary>
/// Protects the AI analyst provider key with a random AES-256 key held in the current macOS user's
/// login Keychain. Only authenticated AES-GCM ciphertext is persisted in notifications.json.
/// </summary>
internal static class AiAnalystCredentialProtection
{
    ...
    private static readonly byte[] FormatMagic = "DAXAI"u8.ToArray();
    private static readonly byte[] AssociatedData =
        "DaxAlgo.Terminal.AiAnalyst.v1"u8.ToArray();
    private static readonly byte[] KeychainService =
        Encoding.UTF8.GetBytes("com.daxalgo.terminal.ai-analyst");
    private static readonly byte[] KeychainAccount =
        Encoding.UTF8.GetBytes("provider-key-encryption-key-v1");
    ...
    public static string? Encrypt(string? plaintext)
    {
        ...
        if (!OperatingSystem.IsMacOS())
        {
            throw new PlatformNotSupportedException(
                "Persistent AI analyst credential protection requires the macOS Keychain.");
        }
    ...
    {
        if (string.IsNullOrEmpty(cipherBase64) || !OperatingSystem.IsMacOS()) return null;
    ...
    private static string DecryptMac(byte[] protectedBytes)
    {
        ...
        if (protectedBytes.Length < headerSize ||
            ...
            protectedBytes[FormatMagic.Length] != FormatVersion)
        {
            throw new CryptographicException("The protected AI analyst credential payload is invalid.");
        }
    ...
    private static byte[] GetOrCreateMacKey(bool createWhenMissing = true)
    {
        ...
        if (!createWhenMissing)
            throw new CryptographicException("The macOS Keychain AI analyst credential key is missing.");
    ...
    private static byte[]? FindMacKey()
    {
        ...
        try
        {
            ...
            if (passwordData == IntPtr.Zero || passwordLength != KeySize)
                throw new CryptographicException("The macOS Keychain AI analyst credential key is invalid.");
    ...
    private static CryptographicException KeychainFailure(int status) =>
        new($"macOS Keychain could not access the AI analyst credential key (status {status}).");
